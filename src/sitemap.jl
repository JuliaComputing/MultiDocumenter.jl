...
function find_sitemap_urls(; docs_root_directory::AbstractString, sitemap_root::AbstractString)
    ...
    walkdocs(docs_root_directory) do fileinfo
        ...
        if !startswith(canonical_href, sitemap_root)
            @warn "Invalid canonical URL, excluded from sitemap." canonical_href
            return
        end
        # Let's make sure we're not adding duplicates
        if !(canonical_href in canonical_urls)
            # TODO: foo/bar, foo/bar/ and foo/bar/index.html are basically equivalent
            # Should be somehow normalize here?
